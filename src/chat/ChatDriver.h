...
public:
  struct Cfg {
    ...
    // Автоприменение: готовые, провалидированные ops из только что полученного
    // ответа — БЕЗ подтверждения пользователя («правки без спроса»).
    // Возвращает отчёт, который драйвер отправляет в чат заметкой.
    std::function<std::string(const std::vector<nlohmann::json>& ops)> onAutoApply;
  };
